...
import { netWorthByCurrency } from './domain/types.ts'
...
function printInitialState(state: InitialState): void {
  ...
  for (const asset of state.assets) {
    const value = 'balance' in asset ? asset.balance : 'cashValue' in asset ? asset.cashValue : asset.marketValue
    console.log(`  [${asset.kind}] ${asset.name}: ${value.toLocaleString()} ${asset.currency}`)
  }
  console.log('\nLiabilities:')
  for (const liability of state.liabilities) {
    console.log(`  [${liability.kind}] ${liability.name}: ${liability.balance.toLocaleString()} ${liability.currency}`)
  }
  console.log('\nNet worth by currency:')
  for (const [currency, amount] of Object.entries(netWorthByCurrency(state))) {
...
async function loadSnapshot() {
  ...
  if (!apiKey || !apiSecret) {
    throw new Error('KUBERA_API_KEY and KUBERA_API_SECRET must be set to use --live (see .env)')
  }
